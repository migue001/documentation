...
:doc:`documentation </applications/sales/subscriptions>` to learn more.

Under the :guilabel:`Recurring Prices` tab, the same functionality of the :guilabel:`Price Rules`
tab is present, with the only difference being that a recurring time period can be applied in the
:guilabel:`Recurring Plan` column.

Click :guilabel:`Add a price rule`, and after :guilabel:`Products`, and potential :guilabel:`Product
Variants`, are selected, select the blank field in the :guilabel:`Recurring Plan` column to reveal a
drop-down menu of pre-designated recurrence periods (e.g. `Monthly`, `Quarterly`, `Weekly`, etc.).

New recurrence periods can also be directly created from this column. To do so, type in the name for
the new :guilabel:`Recurring Plan`, then select :guilabel:`Create` from the resulting drop-down menu
to create the time period, which can be edited later.
...
From this pop-up form, the new recurrence period can be directly configured, with specific
:guilabel:`Details`, :guilabel:`Self-Service`, and :guilabel:`Pricing` options. Then, when the
...
Following that, designate a :guilabel:`Period` of time for the rental rule (e.g. `Daily`, `Hourly`,
etc.).
...
accounts for the unique discount being applied. However, if the :guilabel:`Show public price &
discount to the customer` is selected, the customer sees the actual public price *and* how much they
are saving with this pricelist discount.
