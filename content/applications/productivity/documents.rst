...
To do so, select the workspace to share in the left column of your dashboard and possibly one or
several tags that will be automatically added to the uploaded documents. Then, click the arrow next
to the :guilabel:`Upload` button and :guilabel:`Share`.
...
Switch on the :guilabel:`Allow upload` button to allow users to upload files from your workspace.
...
.. note::
   ...
   #. In the :guilabel:`General Settings` page, scroll down to the :guilabel:`Discuss` section and
      go to the :guilabel:`Alias Domain`;
